...
if (data.url) {
const updatedUser = { ...user, profile_picture: `${data.url}?t=${Date.now()}` };
...
return (
<> <Header user={user} onLogout={handleLogout} onProfileUpdate={handleProfileUpdate} /> <StatsCards analyticsData={analyticsData} /> <NavigationTabs activeTab={activeTab} setActiveTab={setActiveTab} />
...
```
  {activeTab === 'patients' && (
    <PatientTable
      ...
      doctors={doctors}
      queue={queue}
      setPatients={setPatients} // <-- Optimistic updates enabled
    />
  )}
  {activeTab === 'doctors' && <DoctorTable doctors={doctors} patients={patients} />}
